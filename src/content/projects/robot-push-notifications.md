...
- **根因**:每小時統計 cron 每算完一個區間就 insert 一列(每區間要掃數月的勝分報表資料,隨區間累積越來越慢);推播用「最大區間結束時間」挑最新一批統計——**新一批寫入第一列的瞬間就成為「最新」**,推播讀到寫一半的批次。半份資料推送照樣 HTTP 200,所以沒有任何 error。
- **為什麼難查(三重迷霧)**:
...
- **追打的第二個坑——DB 主機時鐘偏移,「完成時間」早於「開始時間」**:「寫入時刻」欄一開始用 `DEFAULT CURRENT_TIMESTAMP`(由 DB 蓋章),結果 QA 出現寫入時刻(13:01:56)比區間結束時間(13:02:15)**早 19 秒**的倒掛。用 NTP 當裁判實測:
  ...
  **問題根源:QA 的 MySQL 主機時鐘慢了約 30 秒**
  ...
  倒掛 19 秒 = 30 秒偏移 −(統計耗時 ~10 秒),完全吻合。區間結束時間用 PHP 的錶、寫入時刻用 DB 的錶,兩支錶不同步就倒掛。量測方法:PHP 前後夾擊 `SELECT NOW()` 量 PHP 與 DB 的相對偏移,再用 `w32tm /stripchart` 對 NTP 量絕對偏差。修法:寫入時刻改由 **PHP 於 insert 前一刻蓋章**,與區間結束時間同一時鐘來源,相減恆為真實耗時;錶準的環境(正式機)行為不變,schema 的 DEFAULT 留給其他寫入端當保底。附帶發現:QA 那台 MySQL 上**所有** `NOW()`/`CURRENT_TIMESTAMP` 欄位都慢 30 秒,已列 infra 對時待辦。
