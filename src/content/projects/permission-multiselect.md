...
- 新建權限關聯中間表(帳號表主鍵 + 帳號字串 + 角色群組號,並對「帳號主鍵 + 群組號」建唯一索引),一個帳號可綁定多個角色群組。
- 後端實作**權限合併演算法**:將多個 rule 字串的 suffix 字元做 union,合併後把**合併結果字串**一次傳給前端,前端不再需要自行解析 rule。
...
| 面向 | 改前 | 改後 |
|------|------|------|
| 單一帳號最多角色數 | 1 | 不限(唯一索引去重) |
| 複合需求需新建角色 | 必須 | 不需要,選已有角色組合即可 |
| 前端 rule 解析邏輯 | 前端自行查表 | 後端拼好合併結果字串直接用 |
| 升級停機時間 | — | 0(Seeder 熱升級) |
...
| 權限關聯中間表 | **新增** | 帳號 ↔ 角色多對多;唯一索引(帳號主鍵 + 群組號)防重複 |

```sql
-- 示意 schema(表名與欄位以角色名表示,非實際命名)
CREATE TABLE account_role_map (
  id         BIGINT AUTO_INCREMENT PRIMARY KEY,
  account_sn INT NOT NULL,            -- 帳號表主鍵
  account    VARCHAR(...) NOT NULL,   -- 帳號字串
  role_group INT NOT NULL,            -- 角色群組編號
...
| 方法職責 | 功能 |
|------|------|
| 讀取帳號群組 | 查中間表;若空則 fallback 讀既有角色欄位 |
| 寫入帳號群組 | Transaction 清舊寫新 + 同步既有角色欄位(取群組清單第一個) |
| 權限合併 | 讀多個角色的 rule,union suffix 字元後重組 |
| 群組名稱對照 | 回傳群組號 → 名稱對照 map |
...
| 元件 | 異動 |
|------|------|
| 帳號列表頁 | 改讀群組清單;權限欄改顯示群組名稱 join ", " |
| 群組選擇元件 | 移除「未指派(-1)」選項;停用舊的單一權限欄位(功能改由複選取代) |
...
- 中間表的 unique key 設計需兼顧帳號表主鍵與帳號字串,避免同一帳號對應不同主鍵的罕見資料造成問題。
...
角色定義表對「全部權限」存的 rule 是 `"all"`(不含底線),但權限合併演算法照 `key_suffix` 格式解析:`"all"` 無底線 → key = "all"、suffix = [] → 重組成 `"all_"`。前端驗證因為預期 `"all"` 而失敗。解法是在合併結果做一次特判,把 `["all_"]` 還原成 `["all"]`——字串格式演算法遇到不照格式走的特殊值時,直接特判會比硬套通用邏輯更穩。
...
- 後台登入後的權限載入目前仍只讀既有角色欄位 + 單一角色的 rule;若要讓多角色真正生效,登入流程需改為先讀中間表取得群組清單、再走權限合併演算法。
...
- 帳號列表 API → 回傳群組清單、群組名稱清單、合併後的 rule 字串。
- 建立 / 編輯帳號 API → 接收群組清單,觸發「寫入帳號群組」。
