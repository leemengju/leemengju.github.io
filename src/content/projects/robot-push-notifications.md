...
推送規模:LINE 每日 4 次 + Slack 每小時,至 2026-07 累計 **506 個推送 event**,單則約 1,500 字(切則上限每則 ≤4,900 字);每次區間統計都要掃日統計表數月的資料量。2026-08 起再納入一款麻將類遊戲的在桌真人數,依兩種計價幣別即時計數(見[後續增量](#後續增量))。
...
2. **測試與正式環境都有防火牆**:外部打不進內網 → 改走 Cloudflare 固定網域 + cloudflared tunnel(見上方路線)。
...
- **症狀**:正式機 LINE 定時推播只送出部分統計內容;Slack 正常、統計頁正常、**手動重推完全正常**、log 一片乾淨;測試環境完全重現不了。
- **根因**:每小時統計 cron 每算完一個區間就 insert 一列(每區間要掃日統計表數月的資料,隨區間累積越來越慢);推播用「最大區間結束時間」挑最新一批統計——**新一批寫入第一列的瞬間就成為「最新」**,推播讀到寫一半的批次。半份資料推送照樣 HTTP 200,所以沒有任何 error。
- **為什麼難查(三重迷霧)**:
  1. 失敗只印 console,排程 `runInBackground()` 把 stdout 丟掉 → 應用 log 永遠乾淨;
  2. 「寫入時刻」欄只是複製「區間結束時間」,DB 沒記實際寫入時刻 → 事後查不出「統計何時寫完」,時間證據湮滅;
  3. 測試環境資料小、統計秒級寫完 → 測試環境永遠正常;直覺被誤導成「截斷/則數上限」問題(實際單則才 ~1,500 字,離 LINE 5,000 上限遠得很)。
- **怎麼破案**:先實測切則邏輯無損 + 歷史 506 個 event 全部單則 → 排除截斷;「手動正常、排程短少」鎖定時序;對照排程 :10/:11 與逐筆 insert 拍板。
- **修法**:改成**鏈式觸發**(統計完成後直接呼叫推播,移除固定時間差的獨立 cron)+ **整批單一多列 INSERT**(原子可見,含 LINE 互動查詢在內的任何讀取端都讀不到半份)+ 每則 push 寫 Log(status/body/字數)+「寫入時刻」欄改記實際寫入時刻(連動重跑刪範圍改用「區間結束時間」,否則歷史重跑會刪不到而產生重複)。
- **追打的第二個坑——DB 主機時鐘偏移,「完成時間」早於「開始時間」**:「寫入時刻」欄一開始用 `DEFAULT CURRENT_TIMESTAMP`(由 DB 蓋章),結果測試環境出現寫入時刻(13:01:56)比區間結束時間(13:02:15)**早 19 秒**的倒掛。用 NTP 當裁判實測:

  **問題根源:測試環境的資料庫主機時鐘慢了約 30 秒**

  | 時鐘 | 對 NTP 標準時間的偏差 | 判定 |
  |---|---|---|
  | 工作機(跑 artisan 的 PHP) | **-2.4 秒** | 準 |
  | 測試環境資料庫主機 | **約 -30 秒** | 慢(元兇) |

  倒掛 19 秒 = 30 秒偏移 −(統計耗時 ~10 秒),完全吻合。區間結束時間用 PHP 的錶、寫入時刻用 DB 的錶,兩支錶不同步就倒掛。量測方法:PHP 前後夾擊 `SELECT NOW()` 量 PHP 與 DB 的相對偏移,再用 `w32tm /stripchart` 對 NTP 量絕對偏差。修法:寫入時刻改由 **PHP 於 insert 前一刻蓋章**,與區間結束時間同一時鐘來源,相減恆為真實耗時;錶準的環境(正式機)行為不變,schema 的 DEFAULT 留給其他寫入端當保底。附帶發現:那台資料庫主機上**所有** `NOW()`/`CURRENT_TIMESTAMP` 欄位都慢 30 秒,已列 infra 對時待辦。

這次事故收斂出幾個一般性原則:「產生資料」與「消費資料」兩個排程用**固定時間差**串接等於埋定時炸彈,資料成長遲早會撞上,要嘛鏈式觸發、要嘛寫入原子化,最好兩者都做;多列資料要有「整組完成」的概念,逐筆 insert 的中間態對所有讀取端都可見;`runInBackground()` 的指令裡 console error 等於不存在,重要結果一律走 `Log::`;資料表要留「實際寫入時刻」欄位,否則事後無從鑑識;「測試環境正常、正式機異常」優先懷疑**資料量/時序差異**而非程式碼版本差異;而要拿來相減比較的時間欄位必須來自同一時鐘來源,跨主機(PHP vs DB)各自蓋章,時鐘偏移會做出「倒因為果」的資料,懷疑時鐘問題時就用 NTP 當裁判,別互相猜。
...
- **怎麼發現**:卡很久,最後**重裝時才報錯**,才發現執行用的 yml 在別處;修正後成功透過 cloudflared + 內網主機打進測試與正式環境。
...
- **推送失敗處理現況——尚無自動重試**:失敗依賴上述 Slack error 告警,已足以人工介入;自動重試列在未來規劃。
- **知識傳承**:操作邏輯與架構已整理成內部文件,納入團隊知識庫,全團隊可查。

## 後續增量

**2026-08｜麻將類遊戲在桌人數併入即時營運統計與推播**

- **需求**:一款麻將類遊戲上線後,即時營運統計與 Slack / LINE 推播要顯示該遊戲的「在桌真人數」,並依桌的計價幣別分列(C 幣桌與 Q 幣桌;Q 幣是後來為既有系統新增的虛擬貨幣),機器人不計入。
- **資料來源迭代(三版)**:先想在既有的即時分析統計表加欄位 → **否決**(那張表不會有這款遊戲的資料);再試「查遊戲端介面 + 資料庫 join 桌設定」;**定案:串兩支遊戲端介面**——一支取在桌真人(排除機器人)、一支取「桌 → 幣別」對應,後端字串化 join(桌 id 是 64 位元整數,型別不一致會 join 不到)。幣別對應不只信文件,而是在測試環境實際打遊戲端介面反查驗證(桌名本身就寫著幣別,可交叉比對)。
- **附帶修好一個潛在地雷**:共用的 HTTP POST 工具原本**沒有設超時**(等同無限等待),而即時營運統計是高頻入口,遊戲端一慢就會把它吊死。改法是加一個**選填的超時參數**(預設維持原行為,既有呼叫端零影響),新接的兩支介面帶 5 秒;任何失敗回 0/0 並寫 warning log,**不讓單一資料源拖垮整個統計入口**。
- **跨分支部署與合併衝突消除**:同一份修正要落到多個分支,其中一個較舊的分支缺少共用的文字格式化函式、必須做分支特化;而團隊流程最後會把該分支合回主線,特化就會留下一次性衝突。作法是先做**本地試合併實證**(`git merge --no-commit --no-ff` 後 abort,零成本的衝突預檢),把特化版的 fallback 文字風格與來源分支對齊到 byte 級一致,再試合併確認 **0 衝突**。
...
- 跨分支挑選 commit 時若做了分支特化,日後合併必然衝突 → 要嘛把特化區塊的文字對齊來源分支消掉衝突,要嘛預留「取哪側」的解法紀錄;`git merge --no-commit --no-ff` 試合併後 abort 是零成本的衝突預檢。
