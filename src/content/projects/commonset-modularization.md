...
- **主檔縮減 54%**：主檔從 4221 行壓到 1963 行，閱讀與搜尋成本大幅下降；整體從 1 檔拆為 10 檔（4 個主模組 + 5 個 Dialog 元件 + 1 支邏輯 js）。
- **拆出獨立路由模組**：虛寶交易管理、進階設定、遊戲幣與儲值管理各有自己的路由，獨立路由從 1 條增為 4 條，功能邊界清晰，日後可授予不同權限。
...
- **框架立即被驗證**：2026-04 後續的虛擬貨幣後台專案——全新虛擬貨幣(Q 幣)的兩項新設定——直接沿用新框架，各自新增為獨立 Dialog，無需再改主檔結構。拆分策略不是自我宣稱的「架構比較乾淨」，而是在一個當時還不存在的真實需求上得到事後驗證。
...
> [!NOTE]
> **1963 行是 2026-04 的快照，不是終點。** 模組化後主檔仍持續被加入新功能——例如同期的虛擬貨幣新手特殊機制，就往主檔又加了約 +105 行。換言之，若沒有這次拆分，主檔早已遠超 4221 行；54% 的縮減是在「業務持續擴充」的前提下達成的，實際避免的膨脹更大。
...
- **三波各自獨立上線、獨立驗證**：每拆完一波就先上線，確認該模組在真實使用下的行為與拆分前一致，才動下一波；刻意不做「一次全拆、一次上線」的大爆炸式重構（否決理由見下方[關鍵取捨](#關鍵取捨)）。
- **第一波刻意選小範圍試水**：虛寶交易管理只從主檔帶走約 50 行，行數收益極小——這一波的目的不是縮減，而是先用最低風險驗證「抽成獨立模組 + 掛新路由」這條路走得通；模式證實可行後，第二、三波才各自拆出一千行以上的獨立模組。
- **舊 URL 不斷線**：每次新增獨立路由都同步更新路由設定，原有進入路徑保持可用，既有書籤與其他頁面的連結不受重構影響。
...
- **選擇：依功能邊界拆，不按程式結構拆**
  - 理由：功能邊界對應實際業務需求，未來還能依功能授予不同權限；按程式結構拆（methods／computed 各自集中）只有工程意義、對業務無感，也無法對應到「哪個團隊該看哪一塊」。這個決策讓後續的虛擬貨幣後台專案能直接對號入座，新增設定不必再回頭改主檔結構。
